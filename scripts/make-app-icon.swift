...
func resize(_ sourceImage: NSImage, from sourceRect: NSRect, pixels: Int) -> NSBitmapImageRep {
    ...
    let targetSide = CGFloat(pixels) * iconFillRatio
    let targetOrigin = (CGFloat(pixels) - targetSide) / 2
    sourceImage.draw(
        in: NSRect(x: targetOrigin, y: targetOrigin, width: targetSide, height: targetSide),
        from: sourceRect,
        operation: .copy,
        fraction: 1,
